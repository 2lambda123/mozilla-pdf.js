...
// defined in the `src/display/api.js` file. (Also, keep in mind that the
// scroll-events are handled in a `requestAnimationFrame`-callback.)
...
class PDFViewer {
  ...
  update(scrolled = false) {
    ...
    if (
      ...
      timeDelta > RenderHighestPriorityDelay.TIMEOUT
    ) {
      rhp.calls = 0;
